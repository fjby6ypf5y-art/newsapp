...
const allBtnLit = () => page.evaluate(()=>document.querySelector('#srcs-all').classList.contains('on'));
say('label with everything on', await allBtnLabel());
want('button not lit (nothing hidden yet)', await allBtnLit(), false);
...
want('button lit (everything hidden)', await allBtnLit(), true);
await page.click('#srcs-all'); await page.waitForTimeout(400);
